...
function UpdateResources( )
{
	...
	if (resourceTable)
	{
        var building = resourceTable["building"];
	    ...
	    $( "#TotalBuildingText" ).text = isEnemy ? "0/30" : building + "/" + String(30);
    }
}
...
function HidePanels()
{
    ...
    $( "#TotalBuildingText" ).SetHasClass( "hide", playerTeamId == DOTATeam_t.DOTA_TEAM_BADGUYS);
}
...
function OnUpdateGpm( event_data )
{   
    $( "#GpmText" ).SetHasClass("minus", false)
    $( "#GpmText" ).SetHasClass("plus", false)
    $( "#GpmText" ).SetHasClass("GpmText", false)
	$( "#GpmText" ).text = ""
    if(event_data.key2 == true)
	{
		$( "#GpmText" ).SetHasClass("minus", true)
	} 
	else if(event_data.key2 == false)
	{
		$( "#GpmText" ).text = "+"
		$( "#GpmText" ).SetHasClass("plus", true)
	} 
	else 
	{
		$( "#GpmText" ).SetHasClass("GpmText", true)
	}
	$( "#GpmText" ).text = $( "#GpmText" ).text + event_data.key1
}
...
(function()
{
	...
	GameEvents.Subscribe( "update_gpm", OnUpdateGpm);
