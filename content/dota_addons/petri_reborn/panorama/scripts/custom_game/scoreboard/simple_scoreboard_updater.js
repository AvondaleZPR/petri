...
(function () {
	GameEvents.Subscribe( "update_score_player", OnUpdateScore);
...
function OnUpdateScore( event_data )
{
	playerScore[event_data.pid] = event_data.score;
	$.Msg( "OnUpdateScore: ", playerScore[event_data.pid] );
};
...
function _ScoreboardUpdater_UpdatePlayerPanel( scoreboardConfig, playersContainer, playerId, localPlayerTeamId )
{
	...
	var playerScoreLabel = playerPanel.FindChildInLayoutFile( "PlayerScore" );
	...
	if ( playerInfo )
	{
		...
		goldValue = playerScore[playerId]
		//$.Msg("tatata " + goldValue + " " + playerId)
